...
# ============================================================================
#
# Deploys both services to the lexflow-prod VM.
...
#
# ARGUMENTS:
#   VM_HOST — SSH host for lexflow-prod (default: lexflow-prod)
#
# PREREQUISITES:
#   - SSH key-based access to VM_HOST
#   - provision.sh has been run on the VM
...
log_step "[0/6] Verifying SSH connectivity..."
...
# ── Step 1: Deploy Trust Service (Backend) ──────────────────────────────────
# CRITICAL: Trust service MUST be deployed first because web service
# depends on trust DB being migrated.

log_step "[1/6] Pulling latest backend code..."
remote_exec "cd ${BACKEND_DIR} && git pull origin main"
log_success "Backend code updated"

log_step "[2/6] Installing backend dependencies..."
remote_exec "cd ${BACKEND_DIR} && npm install --production"
log_success "Backend dependencies installed"

log_step "[3/6] Running trust DB migrations..."
remote_exec "cd ${BACKEND_DIR} && npx drizzle-kit migrate"
log_success "Trust DB migrations complete"

log_step "[4/6] Restarting trust service..."
remote_exec "cd ${BACKEND_DIR} && pm2 restart lexflow-trust || pm2 start scripts/ecosystem.config.js --only lexflow-trust"
wait_for_health "http://localhost:4000/health" "Trust Service"

# ── Step 2: Deploy Web Service (Frontend) ───────────────────────────────────

log_step "[5/6] Pulling and building frontend..."
remote_exec "cd ${FRONTEND_DIR} && git pull origin main && npm install --production && npm run build"
log_success "Frontend built"

log_step "[6/6] Running web DB migrations and restarting..."
remote_exec "cd ${FRONTEND_DIR} && npx drizzle-kit migrate"
remote_exec "cd ${FRONTEND_DIR} && pm2 restart lexflow-web || pm2 start ${BACKEND_DIR}/scripts/ecosystem.config.js --only lexflow-web"
